...
public class EnterHotelSearch implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(Click.on(LOCATION),
                ...
                Click.on(ROOM),
                WaitUntil.the(SELECTBOX_ROOM.of(dataTable.get("room")), WebElementStateMatchers.isVisible()).forNoMoreThan(30).seconds(),
                Click.on(SELECTBOX_ROOM.of(dataTable.get("room"))),
                Click.on(ADULT),
                Click.on(SELECTBOX_ADULT.of(dataTable.get("adult"))),
                Click.on(CHILDREN),
                Click.on(SELECTBOX_CHILDREN.of(dataTable.get("children"))),
